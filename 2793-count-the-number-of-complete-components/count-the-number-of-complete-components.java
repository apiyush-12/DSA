...
//     void dfs(int node, ArrayList<Integer>[] graph, boolean[] visited, ArrayList<Integer> component) {
//         visited[node] = true;
//         component.add(node);
...
class Solution{
    public int countCompleteComponents(int n, int[][] edges){
        ...
        for(int i=0; i<n; i++){
            if(!visited[i]){
                ...
                int nodes = component.size();
                int degreeSum = 0;
                for(int node : component) degreeSum += graph[node].size();
                int edgesCount = degreeSum;
                if(edgesCount == nodes*(nodes-1)) ans++;
            }
        }
        return ans;
    }
    void dfs(int node, ArrayList<Integer>[] graph, boolean[] visited, ArrayList<Integer> component){
        ...
        component.add(node);
        for(int next : graph[node]){
            if(!visited[next]) dfs(next, graph, visited, component);
        }
    }
}